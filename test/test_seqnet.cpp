...
TEST_F(SeqNetTest, Initialization) {
    ...
    /* We expect to be in the IDLE state, which now first checks for a call on the same floor. */
...
TEST_F(SeqNetTest, IdleToCloseDoorTransition) {
    ...
    /* NO call on the same floor. */
    out = SeqNet_loop(false);
    /* Call is pending. */
    out = SeqNet_loop(true);

    /* We expect to be in the CLOSE_DOOR state, which checks if the door is closed. */
...
TEST_F(SeqNetTest, FullSequence_GroundToFloor2) {
    ...
    /* No call on same floor. PC is incremented. */
    out = SeqNet_loop(false);
    EXPECT_EQ(out.cond_sel, 0);

    /* PC jumps to CLOSE_DOOR. */
    out = SeqNet_loop(true);
    EXPECT_EQ(out.cond_sel, 4);
    EXPECT_EQ(out.req_door_state,false);
